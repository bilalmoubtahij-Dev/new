...
const EnglishTest = () => {
  ...
  const [userInfo, setUserInfo] = useState({ fullName: '', email: '', phone: '+212' });
  ...
  useEffect(() => {
    ...
    setIsFormValid(isPhoneValid && isEmailValid && isNameValid);
  }, [userInfo]);
  ...
  const validateTextWithApi = async (text: string) => {
    const words = text.trim().split(/\s+/).filter(w => w.length > 0);
    if (words.length === 0) return { valid: false, error: "Please enter your answer." };
    for (const word of words) {
      if (word.length >= 2 && !/[aeiouy]/i.test(word)) return { valid: false, error: "Please use real words (too few vowels)." };
      if (/[bcdfghjklmnpqrstvwxz]{5,}/i.test(word)) return { valid: false, error: "Please use real words (too many consecutive consonants)." };
      if (/(.)\1{2,}/i.test(word)) return { valid: false, error: "Please use real words (invalid repeated letters)." };
    }
    let validWords = 0;
    try {
      ...
      validWords = results.filter(Boolean).length;
      if (validWords / words.length < 0.6) return { valid: false, error: "Your answer contains unrecognizable words. Please try again." };
      return { valid: true, error: '' };
    } catch (e) { return { valid: true, error: '' }; }
  };
  ...
  const handleNext = async () => {
    ...
    if (q.type === 'text_input' && typeof ans === 'string') {
      ...
      setValidationError('');
      const result = await validateTextWithApi(ans);
            ...
            <div className="mt-8 border-t border-gray-800 pt-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input type="email" value={userInfo.email} onChange={(e) => setUserInfo({ ...userInfo, email: e.target.value })} className="w-full bg-dark-900 border border-gray-700 rounded-lg p-3 text-white focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-colors" placeholder="e.g. kenza@example.com" />
                </div>
                <div>
    ...
    return (
      <div className="bg-dark-900 min-h-screen pt-24 pb-12 flex flex-col items-center p-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4 max-w-2xl w-full justify-center mt-4">
          <Link to="/programs" state={{ tab: 'language', highlight: 'general-english', testLevel: resultData.level, testResultId: testResultIdRef.current || testResultId }} className="w-full sm:w-auto">
